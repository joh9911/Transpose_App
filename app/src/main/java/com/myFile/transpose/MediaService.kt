    ...
    private inner class CustomMediaSessionCallback: MediaSession.Callback {
        ...
        override fun onCustomCommand(
            ...
            when (customCommand.customAction) {
                CONVERT_URL -> {
                    val bundle =
                        customCommand.customExtras.getParcelable<VideoDataModel>("videoData")
                    if (bundle != null) {
                        startStream(bundle)
                    }
                }
...
//    }
    ...
    fun startStream(videoData: VideoDataModel?){
        ...
        val myVideoId = videoData?.videoId
        myVideoId ?: return
        Log.d("스타트","스트림${myVideoId}")
        val disposable: Disposable = Observable.fromCallable {

            val request = YoutubeDLRequest(myVideoId)
    ...
    private fun setUpVideo(convertedUrl: String, videoData: VideoDataModel?){
